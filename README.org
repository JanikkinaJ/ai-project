:PROPERTIES:
:ID:       e9425c64-42bd-4904-8f34-55cfd5884df1
:END:
#+title: Ai projekt
** Tags:                                                       :leibniz:
...
#+DOWNLOADED: screenshot @ 2024-12-18 13:51:05
[[file:../images/AI_project_-_Solve_8_queens_problem_8x8/2024-12-18_13-51-05_screenshot.png]]
...
#+LATEX_HEADER: \rhead{page \thepage}
#+LATEX_HEADER: \cfoot{By: Janik kinaj}
#+LATEX_HEADER: \renewcommand{\headrulewidth}{0.4pt}
#+LATEX_HEADER: \renewcommand{\footrulewidth}{0.4pt}
#+LATEX_HEADER: \usepackage{titlesec}
#+latex_header: \definecolor{darkblue} {HTML}{0000CC}
#+LATEX_HEADER: \titleformat*{\section}{\Huge\bfseries\color{darkblue}}
#+LATEX_HEADER: \titleformat*{\subsection}{\LARGE\bfseries\color{darkblue}}
#+LATEX_HEADER: \titleformat*{\subsubsection}{\large\bfseries\color{darkblue}}
...
**** Structure
#+begin_box
The actual structure consists of the size saved as an int and a board saved as a vector of ~N~ length. Each Index of the vector represents a row and the value the column that a queen of that row sits on. The vector has a ~value~ or ~None~ at each point representing the column that that queen is in or that no queen has been placed there yet.
#+end_box

#+BEGIN_SRC mermaid :file board_class_diagram.png :exports both
classDiagram
    class Board {
        + usize size
        + Vec<Option<i8>> board
        + new(size: usize) Board
        + get(row: usize) Option<i8>
        + set(row: usize, column: i8) bool
        + unset(row: usize, column: i8) bool
        + check_column(column: i8) bool
        + check_diagonal(row1: usize, col1: i8, row2: usize, col2: i8) bool
        + check_all_diagonal(row: usize, col: i8) bool
...
#+end_box
#+begin_src rust :noweb-ref check_all_daigonal
  ...
  fn check_all_diagonal(&self, row: usize, col: i8) -> bool {
      for q_row in 0..(self.size-1) {
          if let Some(q_col) = self.get(q_row) {//handles the case where q_col is None
              ...
              let col_diff = (col - q_col).abs();
              let row_diff = (row as i8 - q_row as i8).abs();
...
#+begin_src rust :noweb-ref check_valid
  ...
  fn check_valid(&self, row :usize,column: i8) -> bool {
      if self.board.iter().any(|&col| col == Some(column)) {
              return false;
